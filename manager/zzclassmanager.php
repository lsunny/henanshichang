...
if(isset($_POST['old'])){
	$o = $_POST['old'];
	unset($_POST['old']);
	update('hnsc_zzclass',$_POST,"cname='$o'");
	header('location:'.$_SERVER['PHP_SELF']);
}else if(isset($_POST['cname'])){ //添加新类别
	save('hnsc_zzclass',$_POST);
	header('location:'.$_SERVER['PHP_SELF']);
}else if(isset($_GET['delid'])){//删除类别
	$stmt = $m->prepare('delete	from hnsc_zzclass where cname=?');
	$stmt->bind_param('s',$_GET['delid']);
	$stmt->execute();
	$stmt->free_result();
	$stmt->close();
	header('location:'.$_SERVER['PHP_SELF']);
